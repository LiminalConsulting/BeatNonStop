...
>
> **Three surfaces — know which one you're touching:**
> 1. **Public site** (`index.html` + `styles/` + `js/` + images at repo root) → served at `beatnonstop.live/`. Attendee-facing. **Edited via `/staging/` only**, promoted by vote.
> 2. **Plan dashboard** (`plan.html` + `styles/planning.css` at repo root) → served at `beatnonstop.live/plan`. Internal tool for the 3-person team. **Edited live at root directly**, never staged. `/promote` explicitly skips these files.
> 3. **LLM memory / coordination state** (`data/state.json`, `knowledge/*`, `data/inbox.md`, `data/outbox.json`, `data/transcript.md`, `.claude/commands/sync.md`, `worker/`, `ARCHITECTURE.md`, `EVENT_SYSTEM_PROMPT.md`) → not rendered anywhere. Bot + builder only. Never exposed to non-technical team members.
>
> **When the team says "add X to the plan" / "adiciona ao /plan":** edit `plan.html` at root, also update `data/state.json` to keep memory aligned. Bilingual: both `<span data-lang>` blocks. Do not touch `staging/`.
